...
class Word2Vec:
    ...
    def word_count(self):  # 统计词频，过滤低频词
        for total, w in enumerate(self.train_data):
            self.words[w] += 1
            if total % 10000 == 0:
                print('%s, get %s sentences, %s unique words.' %
                      (datetime.datetime.now(), total, len(self.words)))
        self.words = {i: j for i, j in self.words.items() if j >=
                      self.min_count}
        ...
        print('%s, min_count=%s left %s unique words.' %
              (datetime.datetime.now(), self.min_count, self.total_words))

    # 定义loss函数：随机抽样、点乘、交叉熵。
    def random_softmax_loss(self, nb_negative, inputs, targets, weights, biases=None):
        nb_classes, real_batch_size = tf.shape(
            weights)[0], tf.shape(targets)[0]
        negative_sample = tf.random_uniform(
            [real_batch_size, nb_negative], 0, nb_classes, dtype=tf.int32)
        ...
        if biases:
            ...
            sampled_logits = tf.matmul(inputs, sampled_weights, transpose_b=True)[
                :, 0, :] + sampled_biases
        else:
            sampled_logits = tf.matmul(
                inputs, sampled_weights, transpose_b=True)[:, 0, :]
    ...
    def build_model(self):  # 建立模型，比较常规
        ...
        self.embeddings = tf.Variable(tf.random_uniform(
            [self.total_words+1, self.word_size], -0.05, 0.05))
        ...
        if self.model == 'cbow':
            self.input_words = tf.placeholder(
                tf.int32, shape=[None, 2*self.window])
            self.input_vecs = tf.nn.embedding_lookup(
                self.embeddings, self.input_words)
            self.input_vecs = tf.expand_dims(
                tf.reduce_sum(self.input_vecs, 1), 1)
        else:
            self.input_words = tf.placeholder(tf.int32, shape=[None, 1])
            self.input_vecs = tf.nn.embedding_lookup(
                self.embeddings, self.input_words)
        if self.shared_softmax:
            self.loss = self.random_softmax_loss(
                self.nb_negative, self.input_vecs, self.target_words, self.embeddings)
        else:
            self.softmax_weights = tf.Variable(tf.random_uniform(
                [self.total_words+1, self.word_size], -0.05, 0.05))
            self.softmax_biases = tf.Variable(tf.zeros([self.total_words+1]))
            self.loss = self.random_softmax_loss(
                self.nb_negative, self.input_vecs, self.target_words, self.softmax_weights, self.softmax_biases)

    def data_generator(self):  # 数据生成器，用来生成每个batch
        if self.model == 'cbow':
            x, y = [], []
            t = [self.word2id[i] for i in self.train_data if i in self.word2id]
            for i, s in enumerate(t):
                win = t[max(0, i-self.window): i] + \
                    t[i+1: min(len(t), i+self.window)]
                win += [0]*(2*self.window-len(win))
                x.append(win)
                y.append([s])
                if len(x) == self.batch_size or i+1 == len(t):
                    yield i, np.array(x), np.array(y)
                    x, y = [], []
        else:
            x, y = [], []
            t = [self.word2id[i] for i in self.train_data if i in self.word2id]
    ...
    def train_model(self):  # 模型训练部分，主要使用了多进程，一个进程负责生成数据，一个负责训练数据
        ...
        for e in range(self.epochs):
            ...
            while True:
                idx, x, y = queue.get()
                if count % 100 == 0:
                    loss_ = self.sess.run(self.loss, feed_dict={
                                          self.input_words: x, self.target_words: y})
                    print('%s, epoch %s, trained on %s sentences, loss: %s' %
                          (datetime.datetime.now(), e+1, idx, loss_))
                ...
                if idx+1 == self.total_sentences:
                    break
            putting.terminate()
        # 训练完成后，重新初始化变量，使得在没有tf的环境中也能够调用模型
        self.embeddings = self.sess.run(self.embeddings)
    ...
    def log_proba(self, input_words):  # 由输入词预测词的概率分布对数
        input_words = [self.embeddings[self.word2id[w]]
                       for w in input_words if w in self.words]
        if input_words:
            if self.model == 'cbow':  # 对于cbow模型，直接将输入词向量求和然后点乘并softmax，这是很自然的。
                ...
                if self.shared_softmax:
                    ...
                    logits = np.dot(self.softmax_weights,
                                    input_words) + self.softmax_biases
                ...
                for v in input_words:
                    if self.shared_softmax:
                        ...
                        logits = np.dot(self.softmax_weights,
                                        v) + self.softmax_biases
                    logits = np.exp(logits - logits.max())+1e-12
                    logs.append(np.log(logits/logits.sum()))
                log_proba = sum(logs) - np.array([0]+[(len(logs)-1)*(np.log(self.words[self.id2word[i]])-np.log(
                    self.total_word_frequency)) for i in range(1, len(self.words)+1)])
    ...
    def save_model(self, saved_folder):  # 保存模型，模型不可再训练（事实上再训练对于多数人来说没有意义）
        ...
        pickle.dump([self.words, self.word2id, self.id2word, self.model,
                    self.shared_softmax], open(saved_folder+'words.pickle', 'w'))
    ...
    def load_model(self, saved_folder):  # 重新加载模型，但不可再训练
        ...
        self.normalized_embeddings = self.embeddings / \
            (self.embeddings**2).sum(axis=1).reshape((-1, 1))**0.5
        if not self.shared_softmax:
